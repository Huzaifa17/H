...
#     add_notification(
#         f"{username} has been assigned as a moderator by {session['username']}",
...
#     # Fetch the post before updating its status
#     post = posts.find_one({'_id': ObjectId(post_id)})
#     if not post:
#         flash('Post not found', 'error')
...
#     # Add notification with a clickable link to the post
#     add_notification(
#         f"{session['username']} approved the post: {post['title']}",
#         link=url_for('view_topic', post_id=post_id)  # Include a link to the post
#     )
